...
```
curl -X POST http://localhost:8080/helloworld.srv/Helloworld.Call -H "Content-Type:application/json" -d "{"""name""":"""Joe"""}"
```
```
{"message":"Hello Joe"}
```
...
## View service nodes
```
curl http://localhost:8080/helloworld.srv/nodes
```
```
