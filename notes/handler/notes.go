...
import (
	...
	streamPb "github.com/oskiegarcia/go-micro-lessons/mq/proto"
	pb "github.com/oskiegarcia/go-micro-lessons/notes/proto"
	pauth "github.com/oskiegarcia/go-micro-lessons/pkg/auth"
	adminpb "github.com/oskiegarcia/go-micro-lessons/pkg/service/proto"
	"github.com/oskiegarcia/go-micro-lessons/pkg/tenant"
...
func (h *Notes) Create(ctx context.Context, req *pb.CreateRequest, rsp *pb.CreateResponse) error {
	...
	rec := &store.Record{Key: key, Value: b}
...
func (h *Notes) Read(ctx context.Context, req *pb.ReadRequest, rsp *pb.ReadResponse) error {
	...
	err = json.Unmarshal(recs[0].Value, &note)
...
func (h *Notes) Delete(ctx context.Context, req *pb.DeleteRequest, rsp *pb.DeleteResponse) error {
	...
	// now delete it
	if err := st.Delete(key); err != nil && err != store.ErrNotFound {
...
func (h *Notes) DeleteData(ctx context.Context, request *adminpb.DeleteDataRequest, response *adminpb.DeleteDataResponse) error {
	...
	st := store.DefaultStore
	keys, err := st.List(store.ListPrefix(request.TenantId))
	if err != nil {
		return err
	}
